...
## 5. Add your real Instagram reels
On the homepage (`views/index.ejs`), find the `reels-grid` section. Each reel
is an `<iframe>` — replace `REPLACE_WITH_REEL_ID_1` (and 2, 3) with the real
reel ID from your Instagram reel's URL:

```
Reel URL:    https://www.instagram.com/reel/Cxxxxxxxxxx/
Embed src:   https://www.instagram.com/reel/Cxxxxxxxxxx/embed
```

Add or remove `<div class="reel-embed">...</div>` blocks to show more or
fewer reels — the grid adjusts automatically (3 → 2 → 1 column on smaller
screens).

## 6. Update social links
Instagram, Facebook and WhatsApp icons appear in the header and footer
(`views/partials/header.ejs` and `footer.ejs`). Update the `href` values:
...
- Add real Instagram reel links (see above) — required for the homepage
  reels section to work.
